...
Процедура ПриОткрытииФормы() Экспорт
	...
	Страницы = ФормаВыбораНастроек.Элементы.Добавить("Страницы1", "ГруппаФормы", Неопределено);
	Страницы.Вид = ФормаВыбораНастроек.ВидГруппыФормы.Страницы;
	Страницы.Заголовок = "Страницы";
	Страницы.Высота = 120;
	
	Страница1 = ФормаВыбораНастроек.Элементы.Добавить("Страница1", "ГруппаФормы", Страницы);
	Страница1.Вид = ФормаВыбораНастроек.ВидГруппыФормы.Страница;
	Страница1.Заголовок = "Страница1";
	
	Кнопка = ФормаВыбораНастроек.Элементы.Добавить("КнопкаОбновитьСписокРасширений", "КнопкаФормы", Неопределено);
	...
	ПолеФормы.Высота = 400;
	
	Страница2 = ФормаВыбораНастроек.Элементы.Добавить("Страница2", "ГруппаФормы", Страницы);
	Страница2.Вид = ФормаВыбораНастроек.ВидГруппыФормы.Страница;
	Страница2.Заголовок = "Страница1";
	
	ПолеВводаАресХранилищПоУмолчанию = ФормаВыбораНастроек.Элементы.Добавить("АресХранилищПоУмолчанию", "ПолеФормы", Неопределено);
	ПолеВводаАресХранилищПоУмолчанию.Вид = ФормаВыбораНастроек.ВидПоляФормы.ПолеВвода;
	ПолеВводаАресХранилищПоУмолчанию.Заголовок = "Адрес";
	ПолеВводаАресХранилищПоУмолчанию.Значение = ТаблицаБазаПриемник[0].АдресХранилищРасширенийПоУмолчанию;
	
	ЛогинХранилищПоУмолчанию = ФормаВыбораНастроек.Элементы.Добавить("ЛогинХранилищПоУмолчанию", "ПолеФормы", Неопределено);
	...
	ПарольХранилищПоУмолчанию = ФормаВыбораНастроек.Элементы.Добавить("ПарольХранилищПоУмолчанию", "ПолеФормы", Неопределено);
...
Процедура ПриИзменииПолеСоСпискомИсточник() Экспорт
	...
	ИспользуетсяПроверкаРасширений = ЗначениеЗаполнено(ТаблицаБазаИсточник.Получить(ИндексБазыИсточник).ИмяПользователя);
	
	ФормаВыбораНастроек.Элементы.Найти("КнопкаОбновитьСписокРасширений")	.Видимость = ИспользуетсяПроверкаРасширений;
	ФормаВыбораНастроек.Элементы.Найти("ПолеРасширений")					.Видимость = ИспользуетсяПроверкаРасширений;
	ФормаВыбораНастроек.Элементы.Найти("АресХранилищПоУмолчанию")			.Видимость = ИспользуетсяПроверкаРасширений;
	ФормаВыбораНастроек.Элементы.Найти("ЛогинХранилищПоУмолчанию")			.Видимость = ИспользуетсяПроверкаРасширений;
	ФормаВыбораНастроек.Элементы.Найти("ПарольХранилищПоУмолчанию")			.Видимость = ИспользуетсяПроверкаРасширений;
...
Процедура ОбновитьПараметрыПодключенияПоУмолчанию() Экспорт
	...
	АресХранилищПоУмолчанию = ФормаВыбораНастроек.Элементы.Найти("АресХранилищПоУмолчанию");
	АресХранилищПоУмолчанию.Значение = БазаПриемник.АдресХранилищРасширенийПоУмолчанию;
	ЛогинХранилищПоУмолчанию = ФормаВыбораНастроек.Элементы.Найти("ЛогинХранилищПоУмолчанию");
	ЛогинХранилищПоУмолчанию.Значение = БазаПриемник.ИмяПользователяРасширенийХранилищаПоУмолчанию;
	ПарольХранилищПоУмолчанию = ФормаВыбораНастроек.Элементы.Найти("ПарольХранилищПоУмолчанию");
...
Функция ВыполнитьОбработку(НетДоступаККонсолиКластера=Ложь)
	...
	// Выполним первый куок кода, сразу после востанновления бекапа
	Если Не БылиОшибки И ЗначениеЗаполнено(БазаПриемник.ПроизвольныйКодСразуПослеВосстановленияБекапа) Тогда
		...
		Попытка
			...
			ПараметрыПодключенияКБазеИсточника = "Srvr=""" + Запуск1С.ПараметрыЗапуска.АдресКластера + """;Ref=""" + Запуск1С.ПараметрыЗапуска.ИмяБазы + """;Usr=""" + Запуск1С.ПараметрыЗапуска.ИмяПользователя + """;Pwd=""" + Запуск1С.ПараметрыЗапуска.ПарольПользователя + """;";
