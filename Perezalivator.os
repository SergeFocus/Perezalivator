	...
	Для Сч = 0 По АргументыКоманднойСтроки.Количество()-1 Цикл
		...
		Если Лев(Аргумент,1) = "-" Тогда
			...
			Если Лев(Аргумент,6) = "-start" Тогда
				...
				ЗаписатьЛог("--------------------------------------------", Ложь);
				ЗаписатьЛог("ЗАПУСК В АВТОМАТИЧЕСКОМ РЕЖИМЕ " + Аргумент, Ложь);
				ЗаписатьЛог("--------------------------------------------", Ложь);
				Аргумент = СтрЗаменить(Аргумент,"-start ","");
				ПараметрыАвтоЗапуска = СтрРазделить(Аргумент,"-",Ложь);				
				Для Каждого ПарамАвто Из ПараметрыАвтоЗапуска Цикл
					Если СтрНачинаетсяС(ПарамАвто, "t") Тогда
						Для Каждого СтрАдрес Из СтрРазделить(СтрЗаменить(ПарамАвто,"t ",""),",") Цикл
							МассивTelegramIDАвтомат.Добавить(СокрЛП(СтрАдрес));
						КонецЦикла;
					ИначеЕсли СтрНачинаетсяС(ПарамАвто, "s") Тогда
						ИмяБазыИсточникаАвтомат = СокрЛП(СтрЗаменить(ПарамАвто,"s ", ""));
					ИначеЕсли СтрНачинаетсяС(ПарамАвто, "d") Тогда
						ИмяБазыПриемникаАвтомат = СокрЛП(СтрЗаменить(ПарамАвто,"d ", ""));
					Иначе						
						Сообщить("Пример: -start -t ххххххх,ххххххх -s ""Base1"" -d ""Base2""");
						ВызватьИсключение "Не известный аргумент: " + ПарамАвто;
					КонецЕсли;
				КонецЦИкла;
				
				ЗаписатьЛог("Будет отправка сообщений чаты: " + МассивTelegramIDАвтомат.Количество(), Ложь);
