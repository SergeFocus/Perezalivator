	...
	Для Сч = 0 По АргументыКоманднойСтроки.Количество()-1 Цикл
		...
		Если Лев(Аргумент,1) = "-" Тогда
			...
			Если СокрЛП(Аргумент) = "-test" Тогда
				...
				Сообщить("--------------------------------------------");
			КонецЕсли;
			Если СокрЛП(Аргумент) = "-autorun" Тогда
				РежимАвтозапуска = Истина;
				Сообщить("--------------------------------------------");
				Сообщить("ЗАПУСК В РЕЖИМЕ АВТОЗАПУСКА");
				Сообщить("--------------------------------------------");
			КонецЕсли;
		Иначе
			// Файл с настройками
...
//******************************************************************
Функция ВыполнитьАвтозапуск()

	//
	БылиОшибки = Ложь;
	
	// Начало выполнения обработки
	СтрокаДействие  = "Начало выполнения автозапуска:";
	Логирование.ЗаписатьСтрокуЛога(СтрокаДействие);
	Логирование.УвеличитьУровень();
	
	// Проверим допустимость исполнения: в базах Источник и Приемник д.б. одной записи
	Если ТаблицаБазаИсточник.Количество()<>1 или ТаблицаБазаПриемник.Количество()<>1 Тогда
		Логирование.ЗаписатьСтрокуЛога("Для автозапуска Базы Источник и Приемник должны содержать по одной настройке...");
		Возврат Ложь;
	КонецЕсли;
	...
	//
	Результат = ВыполнитьОбработку(НетДоступаККонсолиКластера);
	БылиОшибки = Не Результат;
	
	///
	СтрокаДействие  = "Завершение выполнения автозапуска.";
	Логирование.ЗаписатьСтрокуЛога();
	Логирование.УменьшитьУровень();
	Логирование.ЗаписатьСтрокуЛога(СтрокаДействие);
