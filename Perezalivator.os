...
Перем ЗапускВАвтоматическомРежиме, МассивTelegramIDАвтомат, ИмяБазыИсточникаАвтомат, ИмяБазыПриемникаАвтомат;
//******************************************************************
Процедура Инициализация()
	...
	ЗапускВАвтоматическомРежиме = Ложь;
	МассивTelegramIDАвтомат = Новый Массив;
	////////////////////////////////////////
	...
	////////////////////////////////////////
	// Список выполняемых шагов, можно использовать для отладки конкретных этапок
	...
	ТаблицаБазаИсточник.Колонки.Добавить("Индекс");
	...
	ТаблицаБазаПриемник.Колонки.Добавить("Индекс");
	...
	Для Сч = 0 По АргументыКоманднойСтроки.Количество()-1 Цикл
		...
		Если Лев(Аргумент,1) = "-" Тогда
			...
			Если СокрЛП(Аргумент) = "-test" Тогда
				...
				Сообщить("--------------------------------------------");
			КонецЕсли;
			Если Лев(Аргумент,6) = "-start" Тогда
				ЗапускВАвтоматическомРежиме = Истина;
				ЗаписатьЛог("--------------------------------------------", Ложь);
				ЗаписатьЛог("ЗАПУСК В АВТОМАТИЧЕСКОМ РЕЖИМЕ", Ложь);
				ЗаписатьЛог("--------------------------------------------", Ложь);
				
			ИначеЕсли Лев(Аргумент,2) = "--" Тогда
				Сч = Сч + 1;
				
				Если СтрНачинаетсяС(Аргумент, "--s") Тогда
					ИмяБазыИсточникаАвтомат = СокрЛП(АргументыКоманднойСтроки.Получить(Сч));
				ИначеЕсли СтрНачинаетсяС(Аргумент, "--d") Тогда
					ИмяБазыПриемникаАвтомат = СокрЛП(АргументыКоманднойСтроки.Получить(Сч));
				Иначе						
					Сообщить("Пример: -start --s ""Base1"" --d ""Base2""");
					ВызватьИсключение "Не известный аргумент: " + Аргумент;
				КонецЕсли;
	...
	Если ЗапускВАвтоматическомРежиме Тогда	
		Если ЗначениеЗаполнено(ИмяБазыИсточникаАвтомат) Тогда
			ЗаписатьЛог("База источник " + ИмяБазыИсточникаАвтомат, Ложь);	
		Иначе
			ЗаписатьЛог("Не указана база источник " + ИмяБазыИсточникаАвтомат, Ложь);
			ВызватьИсключение "Не указана база источник " + ИмяБазыИсточникаАвтомат;
		КонецЕсли;
		Если ЗначениеЗаполнено(ИмяБазыПриемникаАвтомат) Тогда
			ЗаписатьЛог("База приемник " + ИмяБазыПриемникаАвтомат, Ложь);	
		Иначе
			ЗаписатьЛог("Не указана база приемника " + ИмяБазыПриемникаАвтомат, Ложь);
			ВызватьИсключение "Не указана база приемника " + ИмяБазыПриемникаАвтомат;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗапускВАвтоматическомРежиме Тогда
		////////////////////////////////////////
	...
	Иначе
		Попытка
			//Ищем базы по имени
			ИндексБД = ТаблицаБазаИсточник.НайтиСтроки(Новый Структура("Имя", ИмяБазыИсточникаАвтомат));
			Если ИндексБД.Количество() = 0 Тогда
				ВызватьИсключение "База источник не найдена в настройках";
			Иначе
				ИндексБазыИсточник = ИндексБД[0].Индекс;
			КонецЕсли;
			ИндексБД = ТаблицаБазаПриемник.НайтиСтроки(Новый Структура("Имя", ИмяБазыПриемникаАвтомат));
			Если ИндексБД.Количество() = 0 Тогда
				ВызватьИсключение "База приемник не найдена в настройках";
			Иначе
				ИндексБазыПриемник = ИндексБД[0].Индекс;
			КонецЕсли;
			Если ПолучитьСписокАктивныхСеансов() Тогда
				
				ВызватьИсключение "Есть активные сеансы в базе приемнка";
			Иначе
				ПриНажатииНаКнопкуОбновитьСписокРасширений();
				ВыполнитьОбработку();
			КонецЕсли;
		Исключение
		...
		ИначеЕсли ТипЗнч(Параметр.Значение) = Тип("Массив") Тогда
			...
			Для Каждого ЭлементМассива Из Параметр.Значение Цикл
				Если Параметр.Ключ = "Источники" Тогда
					...
					СтруктураЗначений.Индекс = ТаблицаБазаИсточник.Количество() - 1;
				ИначеЕсли Параметр.Ключ = "Приемники" Тогда
					СтруктураЗначений = ТаблицаБазаПриемник.Добавить();
					СтруктураЗначений.Индекс = ТаблицаБазаПриемник.Количество() - 1;
...
Процедура ПриОткрытииФормы() Экспорт
	...
	СтраницыНастроекРасширений.Высота = 300;		
	...
	ПолеФормыРасширений.Закрепление = 5;
	...
	// Работая с расширениями-------------------------------------------------
	СтраницаСпискаРасширений = ФормаВыбораНастроек.Элементы.Добавить("СтраницаЛог", "ГруппаФормы", СтраницыНастроекРасширений);	
	СтраницаСпискаРасширений.Вид = ФормаВыбораНастроек.ВидГруппыФормы.Страница;
	СтраницаСпискаРасширений.Заголовок = "Логи";	
	// ПолеЛог
	ПолеЛог = ФормаВыбораНастроек.Элементы.Добавить("ПолеЛог", "ПолеФормы", СтраницаСпискаРасширений);
	ПолеЛог.Вид  = ФормаВыбораНастроек.ВидПоляФормы.ПолеНадписи;
	ПолеЛог.ПоложениеЗаголовка = УправляемыйИнтерфейс.ПоложениеЗаголовка.Нет;	
	

	Если РежимОтладки Тогда
		СтраницаСпискаРасширений = ФормаВыбораНастроек.Элементы.Добавить("СтраницаТаблицаБазаИсточник", "ГруппаФормы", СтраницыНастроекРасширений);	
		СтраницаСпискаРасширений.Вид = ФормаВыбораНастроек.ВидГруппыФормы.Страница;
		СтраницаСпискаРасширений.Заголовок = "Таблица База Источник";	
		ФормаТЗ = ФормаВыбораНастроек.Элементы.Добавить("ТаблицаБазаИсточник", "ТаблицаФормы", СтраницаСпискаРасширений);
		ФормаТЗ.ПоложениеЗаголовка = УправляемыйИнтерфейс.ПоложениеЗаголовка.Нет;
		ФормаТЗ.Закрепление = 5;
		ПровайдерТЗ = Новый Провайдер;
		ПровайдерТЗ.Источник = ТаблицаБазаИсточник;
		ФормаТЗ.ПутьКДанным = ПровайдерТЗ;

		СтраницаСпискаРасширений = ФормаВыбораНастроек.Элементы.Добавить("СтраницаТаблицаБазаПриемник", "ГруппаФормы", СтраницыНастроекРасширений);	
		СтраницаСпискаРасширений.Вид = ФормаВыбораНастроек.ВидГруппыФормы.Страница;
		СтраницаСпискаРасширений.Заголовок = "Таблица База Приемник";	
		ФормаТЗ = ФормаВыбораНастроек.Элементы.Добавить("ТаблицаБазаПриемник", "ТаблицаФормы", СтраницаСпискаРасширений);
		ФормаТЗ.ПоложениеЗаголовка = УправляемыйИнтерфейс.ПоложениеЗаголовка.Нет;
		ФормаТЗ.Закрепление = 5;
		ПровайдерТЗ = Новый Провайдер;
		ПровайдерТЗ.Источник = ТаблицаБазаПриемник;
		ФормаТЗ.ПутьКДанным = ПровайдерТЗ;

		СтраницаСпискаРасширений = ФормаВыбораНастроек.Элементы.Добавить("СтраницаТаблицаРасширений", "ГруппаФормы", СтраницыНастроекРасширений);	
		СтраницаСпискаРасширений.Вид = ФормаВыбораНастроек.ВидГруппыФормы.Страница;
		СтраницаСпискаРасширений.Заголовок = "Таблица Расширений";	
		ФормаТЗ = ФормаВыбораНастроек.Элементы.Добавить("ТаблицаРасширений", "ТаблицаФормы", СтраницаСпискаРасширений);
		ФормаТЗ.ПоложениеЗаголовка = УправляемыйИнтерфейс.ПоложениеЗаголовка.Нет;
		ФормаТЗ.Закрепление = 5;
		ПровайдерТЗ = Новый Провайдер;
		ПровайдерТЗ.Источник = ТаблицаРасширений;
		ФормаТЗ.ПутьКДанным = ПровайдерТЗ;
	КонецЕсли;
	...
	Группа.Закрепление = УправляемыйИнтерфейс.СтильЗакрепления.Лево;
	...
	ОповеститьОНачалеВосстановления.Закрепление = УправляемыйИнтерфейс.СтильЗакрепления.Лево;

	ПолеПробел = ФормаВыбораНастроек.Элементы.Добавить("ПолеПробел", "ПолеФормы", Группа);
	ПолеПробел.Вид  = ФормаВыбораНастроек.ВидПоляФормы.ПолеНадписи;
	ПолеПробел.Значение = "";
	ПолеПробел.ПоложениеЗаголовка = УправляемыйИнтерфейс.ПоложениеЗаголовка.Нет;
	ПолеПробел.Ширина = 100;
	ПолеПробел.Закрепление = УправляемыйИнтерфейс.СтильЗакрепления.Лево;
	...
	Кнопка.Закрепление = УправляемыйИнтерфейс.СтильЗакрепления.Лево;
	...
	Кнопка.Закрепление = УправляемыйИнтерфейс.СтильЗакрепления.Право;
...
Процедура ОтправитьСообщение(ТекстСообщения) Экспорт
	...
	Для Каждого Стр Из ДанныеБазы Цикл
		...
		Мессенджер.ОтправитьСообщение("telegram", Стр.ID, "```"  + Символы.ПС + ТекстСообщения + Символы.ПС + "```", ,"md");
	КонецЦикла;
КонецПроцедуры

Процедура ЗаписатьЛог(СтрокаДействие, ВыводитьНаЭкран = Истина) Экспорт
	Логирование.ЗаписатьСтрокуЛога(СтрокаДействие);
	Если ВыводитьНаЭкран И НЕ ЗапускВАвтоматическомРежиме Тогда
	...
	Иначе
		...
		Иначе
			...
			Для Каждого Расширение Из ТаблицаРасширенийВБазе Цикл
				Строки = ТаблицаРасширений.НайтиСтроки(Новый Структура("ИндексБазыПриемник, Имя", Число(ИндексБазыПриемник), Расширение.Имя));
...
Функция ВыполнитьТестНастроек()
	...
	Для Каждого БазаИсточник ИЗ ТаблицаБазаИсточник Цикл
		...
		Если ТаблицаФайловБД = Неопределено Тогда
			СтрокаДействие = "Получить структуру файлов БД Источника - ОШИБКА: " + УправлениеMSSQL.ТекстОшибки;
			БылиОшибки = Истина;
		ИначеЕсли ТаблицаФайловБД.Количество() = 0 Тогда
			СтрокаДействие = "Получить структуру файлов БД Источника - ОШИБКА: Список файлов пуст!";
			БылиОшибки = Истина;
		Иначе
			СтрокаДействие = "Получить структуру файлов БД Источника - УСПЕШНО";
		КонецЕсли;
		Логирование.ЗаписатьСтрокуЛога(СтрокаДействие);
...
Функция ВыполнитьОбработку(НетДоступаККонсолиКластера=Ложь)
	...
	Если Не НетДоступаККонсолиКластера 
		И НЕ СписокШагов.Найти("Завершать соединения пользователей") = Неопределено И НЕ ЗапускВАвтоматическомРежиме Тогда
		Если ТаблицаПользователи.Количество() > 0 Тогда
	...
	Если НЕ ТаблицаРасширенийВБазе = Неопределено И Не БылиОшибки И НЕ СписокШагов.Найти("Выполнить этапы подключения расширений к хранилищам") = Неопределено Тогда
		Для Каждого Расширение Из ТаблицаРасширенийВБазе Цикл
			Если Расширение.ИспользоватьНастройкиПоУмолчанию = Неопределено Тогда
				СтрокаДействие = "Расширение: " + Расширение.Имя + " - " + "Пропущено! Укажите в настройках параметры хранилища";
				ЗаписатьЛог(СтрокаДействие);
				Продолжить;
			КонецЕсли;
