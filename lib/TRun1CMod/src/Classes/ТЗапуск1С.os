...
Функция ПолучитьСписокРасширений() Экспорт
   ...
   КонецЕсли;
	...
	Попытка
		...
		ПараметрыПодключенияКБазеИсточника = "Srvr=""" + ПараметрыЗапуска.АдресКластера + """;Ref=""" + ПараметрыЗапуска.ИмяБазы + """;Usr=""" + ПараметрыЗапуска.ИмяПользователя + """;Pwd=""" + ПараметрыЗапуска.ПарольПользователя + """;";
