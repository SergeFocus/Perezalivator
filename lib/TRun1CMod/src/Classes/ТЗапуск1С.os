...
Процедура ИнициироватьПараметры()
	...
	ПараметрыЗапуска.Вставить("ИмяРасширения","");
...
//*****************************************************************
// Получает список расширений текущей информационной базой
//
Функция ПолучитьСписокРасширений() Экспорт
 
   БазаБылаНайдена = Ложь;
   ТекстОшибки  = "";
   ПараметрыПодключенияКБазеИсточника = "";
   ...
   КонецЕсли;
	...
	Попытка
		...
		ПараметрыПодключенияКБазеИсточника = 
		"Srvr=""" + ПараметрыЗапуска.АдресКластера + 
		""";Ref=""" + ПараметрыЗапуска.ИмяБазы + 
		""";Usr=""" + ПараметрыЗапуска.ИмяПользователя + 
		""";Pwd=""" + ПараметрыЗапуска.ПарольПользователя + 
		?(ЗначениеЗаполнено(ПараметрыЗапуска.КодРазрешения), """;UC=""" + ПараметрыЗапуска.КодРазрешения,"") + """;";
		
		СоединениеСБазой = Коннектор.Connect(ПараметрыПодключенияКБазеИсточника);
		
		ВсеРасширения = СоединениеСБазой.РасширенияКонфигурации.Get();
		
		Для Каждого Расширение Из ВсеРасширения Цикл
			Нов = ТаблицаРасширений.Добавить();
			Нов.Имя = Расширение.Имя;
			Нов.УникальныйИдентификатор = СоединениеСБазой.String(Расширение.УникальныйИдентификатор);
			Нов.Версия = Расширение.Версия;
			Нов.ХешСумма  = СоединениеСБазой.String(Расширение.ХешСумма);
		КонецЦикла;
   Исключение		
	   ТекстОшибки = ОписаниеОшибки();
	   Возврат Неопределено;
...
//	ПолныйПутьКФайлу - Строка - Полный путь к файлу
//
Функция ЗагрузитьКонфигурациюИзФайла(ПолныйПутьКФайлу, ОбновлятьКонфигурациюПослеЗагрузки = Истина, ПредупрежденияКакОшибки = Ложь, НаСервере=Ложь) Экспорт
	...
	Попытка
		СтрокаЗапуска = 
			...
			?(ЗначениеЗаполнено(ПараметрыЗапуска.ИмяРасширения)," -Extension " + ОбернутьВКавычки(ПараметрыЗапуска.ИмяРасширения) + " ", "") +
			?(ОбновлятьКонфигурациюПослеЗагрузки,
				" /UpdateDBCfg " +
...
//*****************************************************************
// Удалить конфигурацию расширения из базы данных
// 
// Параметры:
...
//	
Функция УдалитьКонфигурациюРасширенияБазыДанных(ПредупрежденияКакОшибки=Ложь, НаСервере=Ложь) Экспорт
	...
	// Без имени расширения не работает
	Если НЕ ЗначениеЗаполнено(ПараметрыЗапуска.ИмяРасширения) Тогда
		Возврат Ложь;
	КонецЕсли;
...
//
Функция ОбновитьКонфигурациюИзХранилища(НомерВерсии="", ПолучатьЗахваченныеОбъекты = Ложь, Форсировать = Истина, ОбновлятьКонфигурациюПослеЗагрузки = Истина,ПредупрежденияКакОшибки=Ложь, НаСервере=Ложь) Экспорт
