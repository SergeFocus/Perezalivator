...
// Получает список текущих сеансов с информационной базой
//
Функция ПолучитьСписокРасширений(ПропускатьКонсольКластера = Истина) Экспорт
   ...
   КонецЕсли;
	...
	Попытка
		...
		ПараметрыПодключения = "Srvr=""" + ПараметрыЗапуска.АдресКластера + """;Ref=""" + ПараметрыЗапуска.ИмяБазы + """;Usr=""" + ПараметрыЗапуска.ИмяПользователя + """;Pwd=""" + ПараметрыЗапуска.ПарольПользователя + """;";
		
		СоединениеСБазой = Коннектор.Connect(ПараметрыПодключения);
		
		ВсеРасширения = СоединениеСБазой.РасширенияКонфигурации.Получить();
		
		Для Каждого Расширение Из ВсеРасширения Цикл
			Нов = ТаблицаРасширений.Добавить();
			Нов.Имя = Расширение.Имя;
			Нов.УникальныйИдентификатор = Расширение.УникальныйИдентификатор;
			Нов.Версия = Расширение.Версия;
			Нов.ХешСумма  = Расширение.ХешСумма ;
		КонецЦикла;
   Исключение		
	   ТекстОшибки = ОписаниеОшибки();
	   Возврат Неопределено;
...
Функция СохранитьКонфигурациюИзХранилищаВФайл(ПолныйПутьКФайлу, НомерВерсии="") Экспорт
	...
	Попытка
		СтрокаЗапуска = 
			...
			ВставитьПараметрЗапуска(ПараметрыЗапуска.ИмяПользователяХранилища,"ConfigurationRepositoryN ") +
			ВставитьПараметрЗапуска(ПараметрыЗапуска.ПарольПользователяХранилища,"ConfigurationRepositoryP ") +
			ВставитьПараметрЗапуска(ПараметрыЗапуска.ИмяРасширения,"Extension") +
