...
12. Пароль польхователя хранилища расширения
...
В этом случае - при восстановлении базы, расширения просто перенесуться как есть.
В случае, если используются хранилища, тогда надо брать варинты DEV_PEREZALIVATOR_DEST и DEV_PEREZALIVATOR_SOURCE.
...
1. Есть папка, где лежат все хранилища расширений,  и имена папок соответствют именам расширений
...
10. Если указаны настройки подключения к хранилищам расширений то стартует цикл работы с расширениями: 
	11. Отключение расширений базы-приемника от хранилища
	12. Подключение расширений базы-приемника к хранилищу
	13. Обновление расширений конфигурации базы данных базы-приемника
14. Снятие блокировки регламентных заданий и начала сеансов базы-приемника
15. Уведомление о результате по электронной почте (если указан smtp адрес, иначе шаг пропускается)

Дополнительно: в 10 пункте идет проверка на то, указан ли *АдресХранилища* в параметрах, если нет, то проверяется включен ли флаг *ИспользоватьНастройкиПоУмолчанию* и указан ли *АдресХранилищРасширенийПоУмолчанию*, если нет, то не будет выполнена попытка работы с хранилищем у данного расширения.
Т.е. если есть несколько расширений, у которых нет хранилища, их можно или вообще не указывать в параметрах, или же указывать, но ставить флаг *ИспользоватьНастройкиПоУмолчанию* в Ложь, и не указывать путь к хранищу.
